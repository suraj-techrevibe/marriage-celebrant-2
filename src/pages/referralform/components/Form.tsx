...
const formSchema = z.object({
  ...
  recommend: z.enum(['Yes', 'No', 'Maybe'], {
    message: "Please select an option", // use message instead of errorMap
  }),
...
const Form = () => {
  ...
  const onSubmit = async (data: { [s: string]: unknown; } | ArrayLike<unknown>) => {
...
.catch(err => console.log(err.response || err));
  ...
  return (
    <section
    className="relative bg-slate-50 py-12 px-4"
    style={{
  ...
  >
  <div className="max-w-3xl mx-auto bg-white  shadow-sm border border-gray-100 p-8 md:p-12">
    <h1 className="text-3xl font-bold text-[#1a2b5a] mb-8 text-center">
      New Participant Referral Form
    </h1>

    <form className="space-y-6 text-left" onSubmit={handleSubmit(onSubmit)}>
      <h2 className="text-xl text-center font-bold text-[#1a2b5a] border-b pb-6">Participant's Details:</h2>
        ...
        <div className="flex space-x-4 justify-start">
          {['Yes', 'No', 'Maybe'].map((opt) => (
            <label key={opt} className="flex items-center space-x-2 cursor-pointer">
              <input type="radio" value={opt} {...register("recommend")} className="w-4 h-4 text-red-600" />
              <span>{opt}</span>
            </label>
          ))}
        </div>
        ...
        <div className="overflow-x-auto border border-gray-200 ">
          <table className="w-full text-sm text-left">
            <thead className="bg-slate-100 text-gray-700 border-b">
              <tr>
                <th className="p-2 border-r text-left">#</th>
                <th className="p-2 border-r text-left">Full Name</th>
            ...
            <tbody>
              {fields.map((field, idx) => (
                <tr key={field.id} className="border-b">
                  <td className="p-2 border-r bg-slate-50 font-bold text-left">{idx + 1}</td>
                  <td className="p-1 border-r"><input {...register(`references.${idx}.name`)} className="w-full p-1 focus:outline-none bg-transparent text-left" /></td>
                  <td className="p-1 border-r"><input {...register(`references.${idx}.address`)} className="w-full p-1 focus:outline-none bg-transparent text-left" /></td>
                  <td className="p-1"><input {...register(`references.${idx}.contact`)} className="w-full p-1 focus:outline-none bg-transparent text-left" /></td>
                </tr>
              ))}
            </tbody>
          </table>
      ...
      <div className="pt-6 text-left">
        <button type="submit" disabled={isSubmitting} className={`${isSubmitting ? 'bg-gray-400' : 'bg-red-700'} text-white px-12 py-3  font-bold text-lg hover:bg-[#f23319] transition-colors`}>
